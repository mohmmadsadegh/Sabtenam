from tkinter import Tk, Label, Button, Checkbutton, Radiobutton
from tkinter import messagebox, Entry, Frame, ttk, PhotoImage
...
def OnClickRigester():
    ...
    age = Age.get()
    avrage = Avrage.get()
    Information = {"Name": name, "Famili": famili,
                   "Age": age, "Avrage": avrage}
    ...
    if float(user["Avrage"]) >= 15 and float(user["Avrage"]) <= 20 and meliyat.get() == 1 and jens.get() == 2 and moafiyat.get() == 1:
        ...
        if float(user["Avrage"]) < 15:
            messagebox.showwarning("توجه",
                                   "معدل شما کم تر از حد نصاب دانشگاه است")
            return False
        elif meliyat.get() != 1:
...
def insertDATA(value):
    tbl.insert("", "end", text="1", values=[Avrage.get(), Age.get(),
                                            Famili.get(), Name.get()])
...
def LoadDeta(value):
    for item in value:
        tbl.insert("", "end", text="1", values=[item["Name"], item['Famili'],
                                                item['Age'], item['Avrage']])


def OnClickDelete():
    result = messagebox.askquestion("حذف",
                                    "ایا داده مورد نظر را می خواهید حذف کنید")
    print(result)
    if result == "yes":
        Delete()
...
def Delete():
    ...
    if selection_row != ():
        ...
        dic = {'Name': selectionitem[3], 'Famili': selectionitem[2],
               'Age': str(selectionitem[1]), "Avrage": str(selectionitem[0])}
...
def OnClickEdit():
    ...
    print(userindex)
    mydic = {'Name': selectionitem[3], 'Famili': selectionitem[2],
             'Age': str(selectionitem[1]), "Avrage": str(selectionitem[0])}
    print("userAll:", userAll)
    print("mydic:", mydic)
    if selection_row != ():
        tbl.item(selection_row, values=[Avrage.get(), Age.get(),
                                        Famili.get(), Name.get()])
        listitems = [Name, Famili, Age, Avrage]
        Clear(listitems)        
...
#  button
btnRigester = Button(screen, text="ثبت نام", bg="green", fg="black",
                     command=OnClickRigester).place(x=770, y=188)
btnSearch = Button(frmSearch, text="جستجو کنید",
                   command=OnClickSearch).place(x=0, y=30)
btnDelete = Button(screen, text="حذف", justify="right",
                   bg="red", fg="black", command=OnClickDelete)
btnDelete.place(x=825, y=188)
btnEdit = Button(screen, text="ویرایش", justify="right",
                 bg="orange", fg="black", command=OnClickEdit)
#  btnEdit.place(x=870, y=188)
btnFrmSearch = Button(screen, text="نمایش جستجو", command=ShowSearch)
btnFrmSearch.place(x=770, y=160)
btnFrmClose = Button(frmSearch, text="*", bg="black",
                     fg="white", command=closeFrm, image=bgClose)
...
chek2 = IntVar()
chek3 = IntVar()
check = Checkbutton(screen, text="سابقه قهرمانی",
                    variable=chek1).place(x=650, y=250)
check = Checkbutton(screen, text="کشوری",
                    variable=chek_gharamani1).place(x=590, y=250)
check = Checkbutton(screen, text="اسیایی",
                    variable=chek_gharamani2).place(x=530, y=250)
check = Checkbutton(screen, text="جهانی",
                    variable=chek_gharamani3).place(x=470, y=250)
#  نوع شغل
check = Checkbutton(screen, text="شاغل", variable=chek2).place(x=680, y=300)
shoghl = IntVar()
shoghlDolati = Radiobutton(screen, text="دولتی",
                           variable=shoghl, value=1).place(x=620, y=300)
shoghlAzad = Radiobutton(screen, text="ازاد",
                         variable=shoghl, value=2).place(x=570, y=300)
#  برای مردها معافیت داشتن
moafiyat = IntVar()
Dashtan_moafiat = Radiobutton(screen, text="داشتن معافیت",
                              variable=moafiyat, value=1).place(x=640, y=350)
FaghedSharayet = Radiobutton(screen, text="فاقد معافیت ",
                             variable=moafiyat, value=2).place(x=550, y=350)
#  radiobutton
tahol = IntVar()
...
meliyat = IntVar()
# زن یا مرد بودن
radio_mard = Radiobutton(screen, text="اقا",
                         variable=jens, value=2).place(x=670, y=0)
radio_zan = Radiobutton(screen, text="خانم", variable=jens,
                        value=1).place(x=710, y=0)
#  مجرد بودن یا متاهل بودنl
radio_mojard = Radiobutton(screen, text="مجرد", variable=tahol,
                           value=1).place(x=650, y=50)
radio_Motahel = Radiobutton(screen, text="متاهل", variable=tahol,
                            value=2).place(x=710, y=50)
#  ملیت
radio_irani = Radiobutton(screen, text="ایرانی", variable=meliyat,
                          value=1).place(x=710, y=100)
radio_Ghyre_irani = Radiobutton(screen, text="غیر ایرانی",
                                variable=meliyat, value=2).place(x=640, y=100)
#  پیروان ادیان
raddio_shie = Radiobutton(screen, text="شیعه", variable=din,
                          value=1).place(x=710, y=150)
raddio_soni = Radiobutton(screen, text="سنی", variable=din,
                          value=2).place(x=660, y=150)
raddio_masihi = Radiobutton(screen, text="مسیحی", variable=din,
                            value=3).place(x=600, y=150)
raddio_yahoodi = Radiobutton(screen, text="یهودی", variable=din,
                             value=4).place(x=540, y=150)
raddio_zartoshti = Radiobutton(screen, text="زرتشتی", variable=din,
                               value=5).place(x=480, y=150)
raddio_sayeradyan = Radiobutton(screen, text="سایرادیان", variable=din,
                                value=6).place(x=410, y=150)
#  مقطعه تحصیلی
radio_Kardani = Radiobutton(screen, text="کاردانی", variable=Maghtae_tahsili,
                            value=1).place(x=700, y=200)
radio_Karshenasi = Radiobutton(screen, text="کارشناسی",
                               variable=Maghtae_tahsili,
                               value=2).place(x=630, y=200)
radio_KarshenasiArshad = Radiobutton(screen, text="کارشناسی ارشد",
                                     variable=Maghtae_tahsili,
                                     value=3).place(x=530, y=200)
radio_Doctora = Radiobutton(screen, text="دکترا", variable=Maghtae_tahsili,
                            value=4).place(x=470, y=200)
#  نمودارهای درختی و سطر و ستون
tbl = ttk.Treeview(screen, show="headings", columns=("c1", "c2", "c3", "c4"),
                   height=50)
...
userMenu.add_command(label="اخراج دانشجو", command=PRT, background="black",
                     foreground="white")
...
MenuGetmony.add_command(label="تسویه", command=money_situation,
                        background="green")
